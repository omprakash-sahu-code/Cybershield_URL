...
//initializing counters on landing page load sequence here
(function initIndexCounters() {
  const history = JSON.parse(localStorage.getItem('cybershield_history')) || [];
  if (document.getElementById('totalScans')) {
    document.getElementById('totalScans').textContent = history.length;
    document.getElementById('safeCount').textContent = history.filter(i => i.status === 'safe').length;
    document.getElementById('dangerCount').textContent = history.filter(i => i.status === 'danger').length;
  }
})();
